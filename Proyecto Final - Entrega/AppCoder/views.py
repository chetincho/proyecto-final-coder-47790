...
def editarPerfil(request):
    ...
    if request.method == 'POST':
        ...
        if miFormulario.is_valid():
            ...
            usuario.first_name= informacion['first_name']           # Cambio 13/12 18:49 
            usuario.last_name= informacion['last_name']             # Cambio 13/12 18:55 en compose con la linea de arriba 
            usuario.save()
            
            return render(request, "AppCoder/inicio.html")
    else:
        miFormulario = UserEditForm(initial={'email':usuario.email, 'first_name':usuario.first_name, 'last_name':usuario.last_name})               # Cambio 13/12 18:52 
